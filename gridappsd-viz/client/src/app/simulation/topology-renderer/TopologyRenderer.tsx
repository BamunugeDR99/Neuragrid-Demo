...
export class TopologyRenderer extends Component<Props, State> {
  ...
  private readonly _symbolDimensions: Partial<Record<NodeType, { width: number; height: number }>> = {
  ...
  private _resolveSymbolDimensions() {
    ...
    for (const [type, scalingFactor] of Object.entries(this._scalingFactorPerSymbolTable)) {
      const symbolType = type as keyof typeof this._symbolDimensions;
      this._symbolDimensions[symbolType].width /= scalingFactor;
      this._symbolDimensions[symbolType].height /= scalingFactor;
    }
    if (this.isModelLarge()) {
  ...
  private _invert9500Model(nodeMap: Map<string, Node>) {
    ...
    nodeMap.forEach(node => {
      ...
      if ('y2' in node && typeof node['y2'] === 'number') {
    ...
    nodeMap.forEach(node => {
      ...
      switch (node.type) {
        case NodeType.SWITCH:
          ...
          (node as any)['midpointX'] = (node.screenX1 + (node as any)['screenX2']) / 2;
          (node as any)['midpointY'] = (node.screenY1 + (node as any)['screenY2']) / 2;
  ...
  private _calculateSymbolTransform(node: Node, nodeNameToEdgeMap: Map<string, Edge>) {
    const nodeWithEdgeProps = node as any;
    const edge = nodeNameToEdgeMap.get(nodeWithEdgeProps.from) || nodeNameToEdgeMap.get(nodeWithEdgeProps.to);
